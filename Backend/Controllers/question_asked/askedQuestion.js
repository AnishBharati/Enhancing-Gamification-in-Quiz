...
exports.add_ask_question = async(req, res) => {
    ...
    jwt.verify(token, secretKey, (err, decoded) => {
        ...
        db.query(checkCode, [classId], (err, result) => {
            ...
            const addQuestion = "INSERT INTO questions_asked (asked_by, asked_to, Question, Option1, Option2, Option3, Option4, correct_option, classId, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            db.query(selectClass, [code], (err, classDetails) => {
                ...
                for (let i = 0; i < idsToCheck.length; i++) {
                    if (idsToCheck[i] == askedto) {
                        userFound = true;
                        const updateData = "UPDATE questions_asked SET correct_option = ?, note = ? WHERE id = ?";
                        ...
                        return res.status(204).json({ message: "You are not asked any question" });
                    }
                }
...
exports.see_ask_question = async (req, res) => {
    const { id, askedto, Question, Option1, Option2, Option3, Option4, correct_option, classId, note } = req.body;
    ...
    jwt.verify(token, secretKey, (err, decoded) => {
        ...
        db.query(checkStudent, [classId], (err, result) => {
            ...
            const checkAskedByorTo = "SELECT * FROM questions_asked WHERE (asked_by=? OR asked_to=?) AND classId=?";
            ...
            db.query(checkAskedByorTo, [userId, userId, code], (err, result) => {
                ...
                if (askedBy == userId || askedTo == userId) {
                    ...
                    return res.status(403).json({ error: "You are not authorized to view this question" });
                }
            });
        });
    });
...
exports.delete_ask_question = async (req, res) => {
    ...
    jwt.verify(token, secretKey, (err, decoded) => {
        ...
        db.query(seleteCode, [classId], (err, result) => {
            ...
            const selectQuestion = "SELECT * FROM questions_asked WHERE id = ? AND classId = ?";
            db.query(selectQuestion, [id, code], (err, result) => {
                ...
                if(userId == askedBy) {
                    const deleteQuestion = "DELETE FROM questions_asked WHERE id = ? AND classId = ?";
                    ...
                    return res.status(200).json({message: "You cannot delete question"});
                }
            })
